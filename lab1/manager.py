...
def create_parser():
    ...
    arg_group.add_argument("-get", metavar=("master_password", "address"), nargs=2)
...
def init(master_password: Tuple):
...
if __name__ == "__main__":
    ...
    mode_dict = {"init": init, "put": put, "get": get}
    ...
    if mode != "init" and not os.path.isfile(storage_name):
        print(
            "Password manager must be initalized first.\nUse -init to initalize.",
            file=sys.stderr,
        )
        sys.exit(1)
